...
const   question=chalk.bgGray,
        ans=chalk.green,
        heading=chalk.blueBright.underline,
        notification=chalk.red;
let characterCount=0,
    wordsWithWhitespace=0,
    showNotification=false;
const   words:any=[];//,
let        characters:any=[];
const userParagraph:{text:string}=await inquirer.prompt({
    name:'text',
    type:'input',
...
if(showNotification!==false){
    if(wordsWithWhitespace>1){console.log(notification(wordsWithWhitespace,'words skipped due to whitespaces'));}
    else{console.log(notification(wordsWithWhitespace,'word skipped due to whitespaces'));}
}
